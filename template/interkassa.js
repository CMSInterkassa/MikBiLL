...
var uid_status = false;

var selpayIK = {
    actForm: interkassa_conf.url_action,
    urlApi: interkassa_conf.url_api,
    selPaysys: function () {

        if(document.querySelector('button.sel-ps-ik') != null && $('input[name="ik_am"]').val() > 0){
            selpayIK.getIkSign(document.forms[0]);
            setTimeout(function() {
                if (uid_status) {
            ...
            if($('input[name="ik_am"]').val() > 0) {
                selpayIK.getIkSign(document.forms[0], 1);
            }
            else{
                alert(interkassa_lang.enter_amount)
            }
        }
    },
    paystart : function (data) {
        ...
        var form = $('form')[0];
        if (!data_array['resultData'] && data_array['resultCode'] != 0) {
            $('input[name="ik_act"]').remove();
            $('input[name="ik_int"]').remove();
            $($('form')[0]).attr('action', selpayIK.actForm).submit()
        }
        else {
    ...
    getIkSign : function(form, submit){
        $.post(selpayIK.urlApi, $(form).serialize())
    ...
    $('.ik-payment-confirmation').click(function (e) {
        ...
        if (!$(pm).find('.radioBtn a').hasClass('active') || ($.inArray(ik_pw_via, checkSelCurrPS) == -1)) {
            ...
            if (ik_pw_via.search('test_interkassa|qiwi|rbk') == -1) {
                ...
                $.post(selpayIK.urlApi, $(form).serialize(), function (data) {
